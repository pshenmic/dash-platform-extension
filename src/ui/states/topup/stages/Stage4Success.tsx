...
import { Button, Text, ProgressStepBar, DashLogo } from 'dash-ui-kit/react'
...
function formatDash (credits: bigint): string {
  return creditsToDash(credits).toLocaleString('en-US', { maximumFractionDigits: 8 })
}
...
export function Stage4Success ({
  ...
  const dashAmount = result != null ? creditsToDash(result.topUpAmount) : null
  const usdValue = dashAmount != null && dashRate != null ? dashAmount * dashRate : null
          ...
          <div className='flex items-center justify-between'>
            <div className='flex items-center gap-2.5'>
              <div className='w-[30px] h-[30px] rounded-full bg-dash-primary-dark-blue/[0.05] flex items-center justify-center'>
                <DashLogo size={14} className='text-dash-brand' />
              </div>
              <Text size='sm' weight='medium'>Dash</Text>
            </div>
            <div className='flex flex-col items-end gap-1'>
              <Text size='sm' weight='medium' className='tracking-[-0.03em]'>
                <span className='font-extrabold text-dash-brand'>+ {formatDash(result.topUpAmount)}</span>
                {' '}Dash
              </Text>
              {usdValue != null && (
