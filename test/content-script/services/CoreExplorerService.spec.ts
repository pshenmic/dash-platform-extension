...
describe('CoreExplorerService', () => {
  ...
  describe('getXpubTransactions', () => {
    ...
    const page = (hashes: string[], nextCursor: string | null): unknown => ({
      resultSet: hashes.map(hash => ({
        hash,
        type: 'CLASSIC',
        blockHeight: 10,
        timestamp: '2026-08-20T10:15:55.000Z',
        amount: '1000',
        confirmations: 3,
        instantLock: '0101',
        chainLocked: true
      })),
      pagination: { limit: hashes.length, nextCursor }
    })

    const respondWith = (...pages: unknown[]): void => {
      for (const body of pages) {
        fetchMock.mockResolvedValueOnce({ status: 200, ok: true, json: async () => body })
      }
    }

    it('posts the xpub and normalises the rows', async () => {
      respondWith(page(['aa'], null))

      const { transactions, nextCursor } = await service.getXpubTransactions(XPUB, 'testnet', 10)

      expect(fetchMock).toHaveBeenCalledWith(`${testnetBase}/xpub/transactions`, expect.objectContaining({ method: 'POST' }))
    ...
    it('follows the cursor until the requested count is collected', async () => {
      respondWith(page(['aa', 'bb'], 'c1'), page(['cc'], 'c2'))

      const { transactions } = await service.getXpubTransactions(XPUB, 'testnet', 3)

      expect(transactions.map(t => t.hash)).toEqual(['aa', 'bb', 'cc'])
      expect(fetchMock).toHaveBeenCalledTimes(2)
      expect(JSON.parse(fetchMock.mock.calls[1][1].body).cursor).toEqual('c1')
    })

    it('starts from a caller supplied cursor', async () => {
      respondWith(page(['aa'], null))

      await service.getXpubTransactions(XPUB, 'testnet', 10, 'fromHere')

      expect(JSON.parse(fetchMock.mock.calls[0][1].body).cursor).toEqual('fromHere')
    })

    it('stops when the cursor stops moving rather than spinning forever', async () => {
      respondWith(page(['aa'], 'same'), page(['bb'], 'same'))

      const { transactions, nextCursor } = await service.getXpubTransactions(XPUB, 'testnet', 100, 'same')

      expect(transactions.map(t => t.hash)).toEqual(['aa'])
      expect(nextCursor).toBeNull()
      expect(fetchMock).toHaveBeenCalledTimes(1)
    })
    ...
    it('stops on an empty page', async () => {
      ...
      const { transactions } = await service.getXpubTransactions(XPUB, 'testnet', 100)

      expect(transactions).toEqual([])
      expect(fetchMock).toHaveBeenCalledTimes(1)
    })

    it('reports a mempool transaction as unconfirmed rather than dropping it', async () => {
      respondWith({
        resultSet: [{ hash: 'aa', type: 'CLASSIC', blockHeight: null, timestamp: null, amount: '5', confirmations: 0, instantLock: '', chainLocked: false }],
        pagination: { nextCursor: null }
      })

      const { transactions } = await service.getXpubTransactions(XPUB, 'testnet', 10)

      expect(transactions[0]).toMatchObject({ blockHeight: null, confirmations: 0, instantLocked: false, chainLocked: false })
    })

    it('throws on a non-ok response', async () => {
      fetchMock.mockResolvedValue({ status: 503, ok: false, json: async () => ({}) })

      await expect(service.getXpubTransactions(XPUB, 'testnet', 10)).rejects.toThrow('HTTP 503')
    })
  })
})
